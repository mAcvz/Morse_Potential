MODULE Control !!!
    ...
    SUBROUTINE Control_Ins()
        ...
        ELSE
            !
            WRITE(*,*) "(ERRORE DI INSERIMENTO: L,N,M,alpha non sono stati inseriti corretamente - ARRESTO)"//NEW_LINE("A")
            STOP
            !
        END IF
        !
        ...
        !
            WRITE(*,*) "OPERAZIONE ANNULLATA - valore di alpha non corretto"//NEW_LINE("A")
            STOP
        END IF 
        !
        IF ( M > 99) THEN                     ! CONTROLLO N° A.VETTORI & A.VALORI STAMPATI
            WRITE(*,*) "ERRORE: n° di autovettori richiesti in fase di stampa ECCESSIVO - ARRESTO"//NEW_LINE("A")
            STOP
            !
        ELSE IF (M > 10 ) THEN                ! CONTROLLO NUMERO PUNTI 1 
            !
            WRITE(*,'(a43,i2,a9)') "WARNING: n° M elevato - VERRANNO GENERATI  ", M + 2,' file.txt'
            WRITE(*,*) "Si desidera continuare?  y/n" //NEW_LINE("A")
            !
            ...
            ELSE
            !
                WRITE(*,*) "OPERAZIONE ANNULLATA - ARRESTO" //NEW_LINE("A")
                STOP
            END IF 
            !
        ELSE IF (N < 200 ) THEN                ! CONTROLLO NUMERO PUNTI 1 
            !
            WRITE(*,*) "ERRORE: n° di punti inferiore al minimo N(min) = 201 " //NEW_LINE("A")
            STOP
            !
